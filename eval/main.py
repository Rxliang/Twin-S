...
def general_Test(drill_path, cam_hand_path, img_path, X_path):
    ...
    # cam_mtx = np.load("intrinsic_params_421/M_l.npy")
    X = np.load(X_path)
    for i in range(len(drill_df)):
        ...
        tempt_image = cv2.imread(img_path)
        ...
        tvecs = t
        cam_dist = np.zeros([5])
        pivot0 = np.array([[-28.22107038, 211.8026734,  60.60522491]]).T
        ...
        F_c2tip = np.vstack((np.hstack((R_cd, c2tip)), np.array([0, 0, 0, 1])))
        F_tip2c = sol.invTransformation(F_c2tip)
        rvecs_tip, _ = cv2.Rodrigues(F_tip2c[:3,:3])
        tvecs_tip = F_tip2c[:3, 3]

        print('c2tip', c2tip, np.linalg.norm(c2tip))
        point0 = np.zeros([3])
        imgpt, jac = cv2.projectPoints(point0, rvecs, pivot, cam_mtx, cam_dist)
        # pix,val,dist = projectWithoutDistortion(cam_mtx, 1920, 1080, np.expand_dims(point, 1))
        # print(pix,val,dist)
        imgpt = np.int32(imgpt).reshape(-1, 2)
        print('imgpt:',imgpt[0])
        # showimg = cv2.circle(tempt_image, imgpt[0], 10, [0, 255, 0])
