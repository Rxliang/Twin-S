...
def image_gen(image_msg, ambf):
    try:
        cv2_img = bridge.compressed_imgmsg_to_cv2(image_msg, "bgr8")
        # Resize the real image to the ambf image size
        if not ambf:
            cv2_img = cv2.resize(cv2_img, (w,h))
        return cv2_img
...
def init_hdf5(args, stereo):
    ...
    t_tip = np.load(args.params + 'drill_pivot_0505.npy')
    R_db_d =np.load(args.params + 'R_db_d_.npy')
    T_db_d = np.vstack([np.hstack([R_db_d, t_tip/1000]),np.array([0,0,0,1]).reshape(1,4)])

    T_p_pb = np.load(args.params + 'phacon2pan_0411.npy')
    T_pb_p = Tinv(T_p_pb)
    T_pb_p[:3, 3] = T_pb_p[:3, 3]/1000
    ...
    metadata.create_dataset("T_db_d", data=T_db_d)
    metadata.create_dataset("T_cb_c", data=T_cb_c)
    metadata.create_dataset("T_pb_p", data=T_pb_p)
    metadata.create_dataset("README", data="All position information is in meters unless specified otherwise. \n"
                                           ...
                                           "T_db_d is the drill calibration result. \n"
                                           "Tcb_c is the hand-eye calibration result. \n"
                                           "T_pb_p is the phantom registration result. \n")
...
def callback(*inputs):
    ...
    # if num_data >= chunk:
    #     spinner_char = spinner[int(time.time()) % len(spinner)]
    #     print(f"Wait for writing... {spinner_char}", end="\r")
    #     return
    ...
    for idx, key in enumerate(keys[1:]):  # skip time
        if 'l_img' == key or 'r_img' == key:
            data[key] = image_gen(inputs[idx], ambf=False)
        if  'segm' == key:
            data[key] = image_gen(inputs[idx], ambf=True)
        if 'depth' == key:
            # print("depth")
...
def write_to_hdf5():
    ...
    for key, value in container.items():
        if len(value) > 0:
            ...
            log.log(logging.INFO, (key, data_group[key].shape))
...
def timer_callback(_):
    ...
    global num_data, f, sum_num_data
    for key, data in data_dict.items():
        container[key].append(data)
    num_data = num_data + 1
    sum_num_data = sum_num_data + 1
    if num_data >= chunk:
...
def setup_subscriber(args):
    ...
    # poses
    for topic in args.pose_topics:
        if topic in active_topics:
            topics += [topic]
            if topic.split('_')[-1] == 'drill':
                pose_drill_sub = message_filters.Subscriber(topic, PoseStamped)
                subscribers += [pose_drill_sub]
                container['pose_drill'] = []
            # elif topic.split('_')[-1] == 'camhand':
            elif topic == '/sync_pose_camhand':
                pose_camhand_sub = message_filters.Subscriber(topic, PoseStamped)
                subscribers += [pose_camhand_sub]
                container['pose_camhand'] = []
            elif topic.split('_')[-1] == 'pan':
                pose_pan_sub = message_filters.Subscriber(topic, PoseStamped)
                subscribers += [pose_pan_sub]
                container['pose_pan'] = []
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--world_adf', default='/home/shc/volumetric_drilling/ADF/world/world.yaml', type=str)
    # parser.add_argument(
    #     '--volume_adf', default='../ADF/volume_171.yaml', type=str)
    parser.add_argument(
        '--stereo_adf', default='/home/shc/volumetric_drilling/ADF/stereo_cameras.yaml', type=str)
    parser.add_argument(
        '--stereoL_topic', default='/sync_limage/compressed', type=str)
    parser.add_argument(
        '--depth_topic', default='/sync_pcd/DepthData', type=str)
    parser.add_argument(
        '--stereoR_topic', default='/sync_rimage/compressed', type=str)
    parser.add_argument(
        '--segm_topic', default='/sync_segm/compressed', type=str)
    parser.add_argument(
        '--pose_topics', default=['/sync_pose_camhand', '/sync_pose_pan', '/sync_pose_drill'], type=list)
    parser.add_argument(
    ...
    main(args)
    print('all data size:', sum_num_data)